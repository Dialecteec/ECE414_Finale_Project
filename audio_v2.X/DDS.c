...
// amplitude change per sample during attack and decay, no change during sustain
...
void __ISR(_TIMER_1_VECTOR, ipl2) T1Int(void){
    ...
    note_time++;
    */
    DAC_data_A = (int)(sineTable[(DDS_phase>>24)&0xff]) + 2048;
    write_dac4822(0, DAC_data_A & 0xfff); // write to channel A, gain = 1
...
void generateTables(void){
    ...
    for (i = 0; i<TABLE_SIZE; i++){
        sineTable[i] = (_Accum) (sin(6.2832*((_Accum)i)/(_Accum)TABLE_SIZE));
/*
        if (i < TABLE_SIZE/2){
